...
![真人 vs 自动化脚本：同一动作各重复 250 次的滑动轨迹叠加](docs/images/00-hero-human-vs-script.png)

> 同一个动作，各重复 **250 次**，把轨迹叠在一起。
> 左边是真人，右边是自动化脚本——**不需要任何算法，肉眼就能看出区别。**
>
> 这个项目做的事，是把这种区别拆成 9 个可量化的维度，
> 让程序不仅能判断"不是人"，还能判断"是哪一种机器"。
...
![8 类工具的滑动轨迹叠加对比](docs/images/00-gallery-all-tools.png)
...
![实验室实拍：机械臂、电容点击器、鼠标点击器在真机上操作](docs/images/00-lab-devices.png)

<sub>实验录像截帧。左：机械臂用摄像头看屏幕、机械爪物理点击；中：电容点击器的硅胶头吸在屏幕上；右：鼠标点击器通过蓝牙 HID 驱动。</sub>
...
![压力波动分位直方图](docs/images/02-pressure-fluctuation.png)
...
![采集 App 实时读数：鼠标点击器的一次点击](docs/images/00-collector-readout.png)
